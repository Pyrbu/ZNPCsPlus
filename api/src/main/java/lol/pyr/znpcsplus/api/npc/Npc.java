...
public interface Npc extends PropertyHolder {
   ...
   * @return The {@link NpcLocation} of this NPC for the player Note: this could be different for
   *     each player only if `look_type` property is set to `PER_PLAYER` or the API is used to spawn
   *     the NPC with a different location/rotation for each player.
   */
  ...
  /**
   * Gets the {@link World} this NPC is in Note: can be null if the world is unloaded or does not
   * exist
   *
  ...
  /**
   * Gets the name of the world this NPC is in Unlike {@link Npc#getWorld()} this will never be null
   *
