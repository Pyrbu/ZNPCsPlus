...
public class MySQLStorage implements NpcStorage {
  private static final Logger logger = Logger.getLogger("MySQLStorage");
  ...
  public MySQLStorage(
    ...
    this.database =
        new MySQL(
            configManager.getConfig().databaseConfig().createConnectionURL("mysql"),
            configManager.getConfig().databaseConfig().username(),
            configManager.getConfig().databaseConfig().password(),
            logger);
    database.load();
  ...
  private void createNpcsTable() {
    if (database.executeUpdate(
            "CREATE TABLE "
                ...
                + "uuid VARCHAR(36), world VARCHAR(128), x DOUBLE, y DOUBLE, z DOUBLE, yaw DOUBLE, pitch DOUBLE, type VARCHAR(128), hologramOffset DOUBLE, hologramRefreshDelay BIGINT)")
        != -1) {
  ...
  private void createNpcsPropertiesTable() {
    if (database.executeUpdate(
            "CREATE TABLE "
                ...
                + " (npc_id VARCHAR(256), property VARCHAR(128), value TEXT, PRIMARY KEY (npc_id, property))")
        != -1) {
  ...
  private void createNpcsHologramsTable() {
    if (database.executeUpdate(
            "CREATE TABLE "
                ...
                + " (npc_id VARCHAR(256), line INT, text TEXT, PRIMARY KEY (npc_id, line))")
        != -1) {
  ...
  private void createNpcsActionsTable() {
    if (database.executeUpdate(
            "CREATE TABLE "
                + TABLE_NPCS_ACTIONS
                + " (npc_id VARCHAR(256), action_id INT, action_data TEXT, PRIMARY KEY (npc_id, action_id))")
        != -1) {
