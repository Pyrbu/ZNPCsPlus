...
public abstract class ReflectionLazyLoader<T> {
  private static final Logger logger = Logger.getLogger("ZNPCsPlus Reflection");
  ...
  public T get() {
    ...
    try {
      ...
      if (strict) {
        logger.warning(
            " ----- REFLECTION FAILURE DEBUG INFORMATION, REPORT THIS ON THE ZNPCSPLUS GITHUB ----- ");
        ...
        logger.warning(
            "Server Version: " + PacketEvents.getAPI().getServerManager().getVersion().name());
