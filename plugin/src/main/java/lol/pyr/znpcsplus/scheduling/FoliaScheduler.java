...
public class FoliaScheduler extends TaskScheduler {
  ...
  public void schedulePlayerChat(Player player, String chat) {
    try {
      ...
      Reflections.FOLIA_EXECUTE_REGION
          .get()
          .invoke(scheduler, plugin, player.getLocation(), (Runnable) () -> player.chat(chat));
  ...
  public void runSyncGlobal(Runnable runnable) {
    try {
      ...
      Reflections.FOLIA_RUN_NOW_GLOBAL
          .get()
          .invoke(scheduler, plugin, (Consumer<Object>) o -> runnable.run());
  ...
  public void runAsyncGlobal(Runnable runnable) {
    try {
      ...
      Reflections.FOLIA_RUN_NOW_ASYNC
          .get()
          .invoke(scheduler, plugin, (Consumer<Object>) o -> runnable.run());
    } catch (IllegalAccessException | InvocationTargetException e) {
      throw new RuntimeException(e);
  ...
  public void runLaterSync(Runnable runnable, long delay) {
    runLaterAsync(() -> runSyncGlobal(runnable), delay);
  }
  ...
  public void runLaterAsync(Runnable runnable, long delay) {
    try {
      Object scheduler = Reflections.FOLIA_GET_ASYNC_SCHEDULER.get().invoke(null);
      Reflections.FOLIA_RUN_DELAYED_ASYNC
          .get()
          .invoke(
              scheduler,
              plugin,
              (Consumer<Object>) o -> runnable.run(),
              delay * 50,
  ...
  @Override
  public void runDelayedTimerSync(Runnable runnable, long delay, long interval) {
    runDelayedTimerAsync(() -> runSyncGlobal(runnable), delay, interval);
  }
  ...
  public void runDelayedTimerAsync(Runnable runnable, long delay, long interval) {
    try {
      Object scheduler = Reflections.FOLIA_GET_ASYNC_SCHEDULER.get().invoke(null);
      Reflections.FOLIA_RUN_AT_FIXED_RATE_ASYNC
          .get()
          .invoke(
              scheduler,
              plugin,
