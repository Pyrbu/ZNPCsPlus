...
public class EncodedByteProperty<T> extends EntityPropertyImpl<T> {
  ...
  protected EncodedByteProperty(
      String name,
      T defaultValue,
      Class<T> clazz,
      int index,
      ByteDecoder<T> decoder,
      EntityDataType<Byte> type) {
  ...
  @SuppressWarnings("unchecked")
  public EncodedByteProperty(
      String name, T defaultValue, int index, ByteDecoder<T> decoder, EntityDataType<Byte> type) {
