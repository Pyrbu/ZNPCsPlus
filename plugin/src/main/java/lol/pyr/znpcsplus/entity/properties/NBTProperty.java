...
public class NBTProperty<T> extends EntityPropertyImpl<T> {
  ...
  private final boolean
      allowNull; // This means that the decoder can have null input, not that the property can be
                 // null
  ...
  public NBTProperty(
      ...
      boolean allowNull,
      EntityDataType<NBTCompound> type) {
  ...
  public NBTProperty(
      String name, T defaultValue, int index, NBTDecoder<T> decoder, boolean allowNull) {
    this(
        name,
        defaultValue,
  ...
  public NBTProperty(
      String name, Class<T> clazz, int index, NBTDecoder<T> decoder, boolean allowNull) {
