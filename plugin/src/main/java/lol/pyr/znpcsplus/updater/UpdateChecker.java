...
public class UpdateChecker extends BukkitRunnable {
  ...
  private static final String GET_RESOURCE =
      "https://api.spigotmc.org/simple/0.2/index.php?action=getResource&id=109380";
  ...
  public void run() {
    ...
    try {
      ...
      if (httpRequest.getResponseCode() == HttpURLConnection.HTTP_OK) {
        ...
        logger.warning(
            "Failed to check for updates: HTTP response code " + httpRequest.getResponseCode());
      }
    } catch (IOException e) {
  ...
  private void notifyConsole() {
    logger.warning(
        "Version " + getLatestVersion() + " of " + info.getName() + " is available now!");
