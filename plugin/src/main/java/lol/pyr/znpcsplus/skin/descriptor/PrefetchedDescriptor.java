...
public class PrefetchedDescriptor implements BaseSkinDescriptor, SkinDescriptor {
  ...
  public static CompletableFuture<PrefetchedDescriptor> forPlayer(
      MojangSkinCache cache, String name) {
    return FutureUtil.exceptionPrintingSupplyAsync(
        () -> new PrefetchedDescriptor(cache.fetchByName(name).join()));
  }

  public static CompletableFuture<PrefetchedDescriptor> fromUrl(
      MojangSkinCache cache, URL url, String variant) {
    return FutureUtil.exceptionPrintingSupplyAsync(
        () -> new PrefetchedDescriptor(cache.fetchByUrl(url, variant).join()));
  }

  public static CompletableFuture<PrefetchedDescriptor> fromFile(
      MojangSkinCache cache, String path) {
