...
public class ZNpcImporter implements DataImporter {
  ...
  public Collection<NpcEntryImpl> importData() {
    ...
    for (ZNpcsModel model : models) {
      ...
      NpcLocation location =
          new NpcLocation(
              oldLoc.getX(), oldLoc.getY(), oldLoc.getZ(), oldLoc.getYaw(), oldLoc.getPitch());
            ...
            for (ZNpcsConversationText text : conv.getTexts()) {
              ...
              for (String line : lines) {
                ...
                InteractionAction action =
                    new MessageAction(
                        adventure, textSerializer, line, InteractionType.ANY_CLICK, 0, totalDelay);
      ...
      for (ZNpcsAction action : model.getClickActions()) {
        ...
        npc.addAction(
            adaptAction(action.getActionType(), t, action.getAction(), action.getDelay()));
      }

      for (Map.Entry<String, String> entry : model.getNpcEquip().entrySet()) {
        EntityPropertyImpl<ItemStack> property =
            propertyRegistry.getByName(entry.getKey(), ItemStack.class);
      ...
      if (model.getSkinName() != null) {
        npc.setProperty(
            ...
            new NameFetchingDescriptor(skinCache, model.getSkinName()));
      } else if (model.getSkin() != null && model.getSignature() != null) {
        npc.setProperty(
            propertyRegistry.getByName("skin", SkinDescriptor.class),
            new PrefetchedDescriptor(new SkinImpl(model.getSkin(), model.getSignature())));
      }

      Map<String, Object> toggleValues =
          model.getNpcToggleValues() == null ? model.getNpcFunctions() : model.getNpcToggleValues();
      if (toggleValues != null) {
        if (toggleValues.containsKey("look")) {
          npc.setProperty(
              propertyRegistry.getByName("look", LookType.class), LookType.CLOSEST_PLAYER);
        }
        ...
        if (toggleValues.containsKey("glow") && (boolean) toggleValues.get("glow")) {
          if (!model.getGlowName().isEmpty())
            try {
              npc.setProperty(
                  propertyRegistry.getByName("glow", DyeColor.class),
                  DyeColor.valueOf(model.getGlowName()));
            } catch (IllegalArgumentException e) {
              npc.setProperty(propertyRegistry.getByName("glow", DyeColor.class), DyeColor.WHITE);
            }
          else npc.setProperty(propertyRegistry.getByName("glow", DyeColor.class), DyeColor.WHITE);
        }
      }
  ...
  private InteractionAction adaptAction(
      String type, InteractionType clickType, String parameter, int cooldown) {
    switch (type.toLowerCase()) {
      case "cmd":
      ...
      case "message":
        return new MessageAction(
            adventure, textSerializer, parameter, clickType, cooldown * 1000L, 0);
      case "server":
