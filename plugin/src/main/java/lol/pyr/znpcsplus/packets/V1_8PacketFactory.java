...
public class V1_8PacketFactory implements PacketFactory {
  ...
  public V1_8PacketFactory(
    ...
    this.displayNameProperty =
        LazyLoader.of(() -> propertyRegistry.getByName("display_name", String.class));
    this.tabListDisplayNameProperty =
        LazyLoader.of(() -> propertyRegistry.getByName("tab_list_display_name", Component.class));
    this.alwaysVisibleInTabProperty =
        LazyLoader.of(() -> propertyRegistry.getByName("always_visible_in_tab", Boolean.class));
  }

  @Override
  public CompletableFuture<Void> spawnPlayer(
      Player player, PacketEntity entity, PropertyHolder properties) {
    return addTabPlayer(player, entity, properties)
        .thenAccept(
            ignored -> {
              createTeam(
                  player,
              ...
              sendPacket(
                  player,
                  new WrapperPlayServerEntityHeadLook(entity.getEntityId(), location.getYaw()));
              sendAllMetadata(player, entity, properties);
              sendAllAttributes(player, entity, properties);
              if (alwaysVisibleInTabProperty == null
                  || !properties.getProperty(alwaysVisibleInTabProperty.get()))
                scheduler.runLaterAsync(
                    () -> removeTabPlayer(player, entity),
                    configManager.getConfig().tabHideDelay());
            });
  }
  ...
  public void spawnEntity(Player player, PacketEntity entity, PropertyHolder properties) {
    ...
    sendPacket(
        player,
        type.getLegacyId(clientVersion) == -1
  ...
  @Override
  public CompletableFuture<Void> addTabPlayer(
      Player player, PacketEntity entity, PropertyHolder properties) {
    if (entity.getType() != EntityTypes.PLAYER) return CompletableFuture.completedFuture(null);
    CompletableFuture<Void> future = new CompletableFuture<>();
    Component displayName =
        tabListDisplayNameProperty != null
                && properties.getProperty(tabListDisplayNameProperty.get()) != null
            ? PapiUtil.set(
                textSerializer, player, properties.getProperty(tabListDisplayNameProperty.get()))
            : Component.text(
                PapiUtil.set(
                    player,
                    configManager
                        ...
                        .replace(
                            ...
                            displayNameProperty != null
                                    && properties.hasProperty(displayNameProperty.get())
                                ? properties.getProperty(displayNameProperty.get())
                                : "")));
    skinned(
            player,
            properties,
            new UserProfile(entity.getUuid(), Integer.toString(entity.getEntityId())))
        .thenAccept(
            profile -> {
              sendPacket(
                  player,
  ...
  public void removeTabPlayer(Player player, PacketEntity entity) {
    ...
    sendPacket(
        ...
        new WrapperPlayServerPlayerInfo(
            ...
            new WrapperPlayServerPlayerInfo.PlayerData(
                null, new UserProfile(entity.getUuid(), null), null, -1)));
  ...
  public void sendEquipment(Player player, PacketEntity entity, Equipment equipment) {
    sendPacket(
        ...
        new WrapperPlayServerEntityEquipment(
            entity.getEntityId(), Collections.singletonList(equipment)));
  }
  ...
  protected CompletableFuture<UserProfile> skinned(
      Player player, PropertyHolder properties, UserProfile profile) {
    if (!properties.hasProperty(propertyRegistry.getByName("skin")))
      return CompletableFuture.completedFuture(profile);
    BaseSkinDescriptor descriptor =
        (BaseSkinDescriptor)
            properties.getProperty(propertyRegistry.getByName("skin", SkinDescriptor.class));
  ...
  public void sendAllAttributes(Player player, PacketEntity entity, PropertyHolder properties) {
    ...
    properties.getAppliedProperties().stream()
        .filter(property -> property instanceof AttributeProperty)
        .forEach(
            property ->
                ((AttributeProperty) property).apply(player, entity, false, attributesList));
    sendPacket(player, new WrapperPlayServerUpdateAttributes(entity.getEntityId(), attributesList));
  }

  @Override
  public void sendAttribute(
      Player player, PacketEntity entity, WrapperPlayServerUpdateAttributes.Property property) {
    sendPacket(
        player,
        new WrapperPlayServerUpdateAttributes(
            entity.getEntityId(), Collections.singletonList(property)));
  }
  ...
  public void updateDisplayName(Player player, PacketEntity entity, Component displayName) {
    ...
    sendPacket(
        ...
        new WrapperPlayServerPlayerInfo(
            ...
            new WrapperPlayServerPlayerInfo.PlayerData(
                displayName, new UserProfile(entity.getUuid(), null), null, -1)));
  }
}
