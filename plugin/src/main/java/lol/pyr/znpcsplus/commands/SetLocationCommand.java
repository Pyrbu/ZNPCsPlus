...
public class SetLocationCommand implements CommandHandler {
  ...
  public void run(CommandContext context) throws CommandExecutionException {
    ...
    npc.setLocation(
        new NpcLocation(x, y, z, npc.getLocation().getYaw(), npc.getLocation().getPitch()));
    context.send(
        Component.text(
            "NPC has been moved to " + x + ", " + y + ", " + z + ".", NamedTextColor.GREEN));
  }
  ...
  public List<String> suggest(CommandContext context) throws CommandExecutionException {
    ...
    if (context.argSize() == 2) return Arrays.asList(String.valueOf(npc.getLocation().getX()), "~");
      ...
      return Arrays.asList(String.valueOf(npc.getLocation().getY()), "~");
    else if (context.argSize() == 4)
      return Arrays.asList(String.valueOf(npc.getLocation().getZ()), "~");
    return Collections.emptyList();
  }

  private static double parseLocation(String input, double current)
