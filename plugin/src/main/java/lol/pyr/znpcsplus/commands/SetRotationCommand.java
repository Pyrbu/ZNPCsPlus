...
public class SetRotationCommand implements CommandHandler {
  ...
  public void run(CommandContext context) throws CommandExecutionException {
    ...
    if (pitch < -90 || pitch > 90) {
      ...
      context.send(
          Component.text(
              "Warning: pitch is outside of the -90 to 90 range. It has been normalized to "
                  + pitch
                  + ".",
    ...
    context.send(
        Component.text(
            "NPC has been rotated to " + yaw + ", " + pitch + ".", NamedTextColor.GREEN));
  }
  ...
  public List<String> suggest(CommandContext context) throws CommandExecutionException {
    ...
    if (context.argSize() == 2)
      return Arrays.asList(String.valueOf(npc.getLocation().getYaw()), "~");
    else if (context.argSize() == 3)
      return Arrays.asList(String.valueOf(npc.getLocation().getPitch()), "~");
