...
public class PropertyRemoveCommand implements CommandHandler {
  ...
  public void run(CommandContext context) throws CommandExecutionException {
    ...
    if (!npc.hasProperty(property))
      context.halt(
          Component.text(
              "This npc doesn't have the " + property.getName() + " property set",
              NamedTextColor.RED));
    ...
    context.send(
        Component.text(
            "Removed property " + property.getName() + " from NPC " + entry.getId(),
  ...
  public List<String> suggest(CommandContext context) throws CommandExecutionException {
    ...
    if (context.argSize() == 2)
      return context.suggestStream(
          context.suggestionParse(0, NpcEntryImpl.class).getNpc().getAllProperties().stream()
              .filter(EntityProperty::isPlayerModifiable)
