...
public class SkinCommand implements CommandHandler {
  ...
  public void run(CommandContext context) throws CommandExecutionException {
    ...
    if (type.equalsIgnoreCase("mirror")) {
      ...
      context.halt(
          Component.text(
              "The NPC's skin will now mirror the player that it's being displayed to",
      ...
      PrefetchedDescriptor.forPlayer(skinCache, name)
          .thenAccept(
              skin -> {
                if (skin.getSkin() == null) {
                  context.send(
                      Component.text(
                          "Failed to fetch skin, are you sure the player name is valid?",
                          NamedTextColor.RED));
                  return;
                }
                ...
                context.send(
                    Component.text(
                        "The NPC's skin has been set to \"" + name + "\"", NamedTextColor.GREEN));
              });
      return;
      ...
      npc.setProperty(
          propertyRegistry.getByName("skin", SkinDescriptor.class),
          new NameFetchingDescriptor(skinCache, name));
      npc.respawn();
      context.halt(
          Component.text("The NPC's skin will now be resolved per-player from \"" + name + "\""));
      ...
      if (!variant.equalsIgnoreCase("slim") && !variant.equalsIgnoreCase("classic")) {
        context.send(
            Component.text(
                "Invalid skin variant! Please use one of the following: slim, classic",
                NamedTextColor.RED));
        return;
      }
      ...
      try {
        ...
        context.send(
            Component.text(
                "Fetching skin from url \"" + urlString + "\"...", NamedTextColor.GREEN));
        PrefetchedDescriptor.fromUrl(skinCache, url, variant)
            .thenAccept(
                skin -> {
                  if (skin.getSkin() == null) {
                    context.send(
                        Component.text(
                            "Failed to fetch skin, are you sure the url is valid?",
                            NamedTextColor.RED));
                    return;
                  }
                  npc.setProperty(propertyRegistry.getByName("skin", SkinDescriptor.class), skin);
                  npc.respawn();
                  context.send(
                      Component.text("The NPC's skin has been set.", NamedTextColor.GREEN));
                });
      ...
      context.send(
          Component.text("Fetching skin from file \"" + path + "\"...", NamedTextColor.GREEN));
      PrefetchedDescriptor.fromFile(skinCache, path)
          .exceptionally(
              e -> {
                if (e instanceof FileNotFoundException
                  ...
                  context.send(
                      Component.text(
                          "A file at the specified path could not be found!", NamedTextColor.RED));
                } else {
                  context.send(
                      Component.text(
                          "An error occurred while fetching the skin from file! Check the console for more details.",
                          NamedTextColor.RED));
          ...
          .thenAccept(
              skin -> {
                ...
                if (skin.getSkin() == null) {
                  context.send(
                      Component.text(
                          "Failed to fetch skin, are you sure the file path is valid?",
                          NamedTextColor.RED));
                  return;
                }
                npc.setProperty(propertyRegistry.getByName("skin", SkinDescriptor.class), skin);
    ...
    context.send(
        Component.text(
            "Unknown skin type! Please use one of the following: mirror, static, dynamic, url"));
  }
  ...
  public List<String> suggest(CommandContext context) throws CommandExecutionException {
    ...
    if (context.argSize() == 2)
      return context.suggestLiteral("mirror", "static", "dynamic", "url", "file");
