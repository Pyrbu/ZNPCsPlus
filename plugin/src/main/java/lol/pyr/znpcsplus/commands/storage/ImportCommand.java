...
public class ImportCommand implements CommandHandler {
  ...
  public void run(CommandContext context) throws CommandExecutionException {
    ...
    if (importer == null)
      context.halt(
          Component.text(
              "Importer not found! Possible importers: "
                  + String.join(", ", importerRegistry.getIds()),
              NamedTextColor.RED));
    ...
    FutureUtil.exceptionPrintingRunAsync(
        () -> {
          if (!importer.isValid()) {
            context.send(
                Component.text(
                    "There is no data to import from this importer!", NamedTextColor.RED));
            return;
          }
          try {
            ...
            context.send(
                Component.text(
                    entries.size() + " npcs have been loaded from " + id, NamedTextColor.GREEN));
          } catch (Exception exception) {
            context.send(
                Component.text(
                    "Importing failed! Please check the console for more details.",
                    NamedTextColor.RED));
