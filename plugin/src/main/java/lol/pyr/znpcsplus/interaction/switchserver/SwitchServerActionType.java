...
public class SwitchServerActionType
    implements InteractionActionType<SwitchServerAction>, InteractionCommandHandler {
  ...
  public String serialize(SwitchServerAction obj) {
    return Base64.getEncoder().encodeToString(obj.getServer().getBytes(StandardCharsets.UTF_8))
        + ";"
        + obj.getCooldown()
        + ";"
  ...
  public void appendUsage(CommandContext context) {
    context.setUsage(
        context.getUsage()
            + " "
            + getSubcommandName()
            + " <id> <click type> <cooldown seconds> <delay ticks> <server>");
  }
