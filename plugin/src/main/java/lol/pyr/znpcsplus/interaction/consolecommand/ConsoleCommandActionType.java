...
public class ConsoleCommandActionType
    implements InteractionActionType<ConsoleCommandAction>, InteractionCommandHandler {
  ...
  public String serialize(ConsoleCommandAction obj) {
    return Base64.getEncoder().encodeToString(obj.getCommand().getBytes(StandardCharsets.UTF_8))
        + ";"
        + obj.getCooldown()
        + ";"
        + obj.getInteractionType().name()
        + ";"
        + obj.getDelay();
  }
  ...
  public ConsoleCommandAction deserialize(String str) {
    ...
    InteractionType type =
        split.length > 2 ? InteractionType.valueOf(split[2]) : InteractionType.ANY_CLICK;
    return new ConsoleCommandAction(
        scheduler,
        new String(Base64.getDecoder().decode(split[0]), StandardCharsets.UTF_8),
        type,
        Long.parseLong(split[1]),
        Long.parseLong(split.length > 3 ? split[3] : "0"));
  }
  ...
  public void appendUsage(CommandContext context) {
    context.setUsage(
        context.getUsage()
            + " "
            + getSubcommandName()
            + " <id> <click type> <cooldown seconds> <delay ticks> <command>");
  }
