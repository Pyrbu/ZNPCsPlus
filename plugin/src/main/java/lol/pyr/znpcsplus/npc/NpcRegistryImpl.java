...
public class NpcRegistryImpl implements NpcRegistry {
  ...
  public NpcRegistryImpl(
    ...
    if (configManager.getConfig().autoSaveEnabled()) {
      ...
      scheduler.runDelayedTimerAsync(
          RepeatingTaskGuard.wrap(Bukkit.getLogger(), "npc-autosave", this::save), delay, delay);
    }
  }
  ...
  public Collection<NpcEntryImpl> getProcessable() {
    synchronized (registryLock) {
      return Collections.unmodifiableCollection(
          npcList.stream().filter(NpcEntryImpl::isProcessed).collect(Collectors.toList()));
    }
  }
  ...
  public NpcEntryImpl getByEntityId(int id) {
    synchronized (registryLock) {
      return npcList.stream()
          .filter(
              entry ->
                  entry.getNpc().getEntity().getEntityId() == id
                      || entry.getNpc().getHologram().getLines().stream()
                          .anyMatch(line -> line.getEntityId() == id))
          .findFirst()
          .orElse(null);
    }
  }
  ...
  public Collection<String> getAllPlayerMadeIds() {
    return getAllModifiable().stream().map(NpcEntryImpl::getId).collect(Collectors.toSet());
  }

  public Collection<String> getModifiableIds() {
    synchronized (registryLock) {
      return Collections.unmodifiableSet(
          npcIdLookupMap.entrySet().stream()
  ...
  public void switchIds(String oldId, String newId) {
    ...
    synchronized (registryLock) {
      ...
      if (oldEntry == null)
        throw new IllegalArgumentException("No npc with id " + oldId + " exists!");
      if (npcIdLookupMap.containsKey(normalizedNewId))
        throw new IllegalArgumentException(
            "An npc with the id " + normalizedNewId + " already exists!");
