...
public class NpcImpl extends Viewable implements Npc {
  ...
  public NpcImpl(
    ...
    hologram =
        new HologramImpl(
            ...
            packetFactory,
            textSerializer,
            location.withY(location.getY() + type.getHologramOffset()));
  }
  ...
  public void setLocation(NpcLocation location) {
    this.location = location;
    playerLookMap.clear();
    playerLookMap.putAll(
        getViewers().stream()
            .collect(
                Collectors.toMap(
                    Player::getUniqueId,
                    player -> new float[] {location.getYaw(), location.getPitch()})));
  ...
  public float getHeadYaw(Player player) {
    return playerLookMap
        .getOrDefault(player.getUniqueId(), new float[] {location.getYaw(), location.getPitch()})[
        0];
  }
  ...
  public void setItemProperty(EntityProperty<?> key, ItemStack value) {
    setProperty(
        (EntityPropertyImpl<com.github.retrooper.packetevents.protocol.item.ItemStack>) key,
  ...
  public Set<EntityProperty<?>> getAppliedProperties() {
    return Collections.unmodifiableSet(propertyMap.keySet()).stream()
        .filter(type::isAllowedProperty)
