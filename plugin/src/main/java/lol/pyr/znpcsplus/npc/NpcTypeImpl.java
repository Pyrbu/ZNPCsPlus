...
public class NpcTypeImpl implements NpcType {
  ...
  private NpcTypeImpl(
      ...
      Set<EntityPropertyImpl<?>> allowedProperties,
      Map<EntityPropertyImpl<?>, Object> defaultProperties) {
  ...
  public Set<EntityProperty<?>> getAllowedProperties() {
    return allowedProperties.stream()
  ...
  protected static final class Builder {
    ...
    public Builder addProperties(String... names) {
      for (String name : names) {
        if (propertyRegistry.getByName(name) == null) {
          // Only for use in development, please comment this out in production because some
          // properties are version-dependent
          // logger.warning("Tried to register the non-existent \"" + name + "\" property to the \""
          // + this.name + "\" npc type");
          continue;
        }
    ...
    public <T> Builder addDefaultProperty(String name, T value) {
      ...
      if (property == null) {
        // Only for use in development, please comment this out in production because some
        // properties are version-dependent
        // logger.warning("Tried to register the non-existent \"" + name + "\" default property to
        // the \"" + this.name + "\" npc type");
        return this;
      }
    ...
    public NpcTypeImpl build() {
      ...
      addProperties(
          ...
          "display_name",
          "permission_required",
          "premission_required_perm",
          "player_knockback",
          "player_knockback_exempt_permission",
          "player_knockback_distance",
          "player_knockback_vertical",
          "player_knockback_horizontal",
          "player_knockback_cooldown",
          "player_knockback_sound",
          "player_knockback_sound_name",
          "player_knockback_sound_volume",
          "player_knockback_sound_pitch");
      ...
      if (EntityTypes.isTypeInstanceOf(type, EntityTypes.ABSTRACT_AGEABLE)
          ...
          || EntityTypes.isTypeInstanceOf(type, EntityTypes.ZOGLIN)) {
        addProperties("baby");
      }
      ...
      if (version.isOlderThan(ServerVersion.V_1_11)
          && EntityTypes.isTypeInstanceOf(type, EntityTypes.SKELETON)) {
      ...
      if (EntityTypes.isTypeInstanceOf(type, EntityTypes.SLIME)
          || EntityTypes.isTypeInstanceOf(type, EntityTypes.PHANTOM)) {
        addProperties("size");
      }
      ...
      if (EntityTypes.isTypeInstanceOf(type, EntityTypes.ABSTRACT_TAMEABLE_ANIMAL)
          && !(version.isNewerThanOrEquals(ServerVersion.V_1_14)
              && type.equals(EntityTypes.OCELOT))) {
      ...
      if (EntityTypes.isTypeInstanceOf(type, EntityTypes.ZOMBIE)) {
        ...
        if (version.isNewerThanOrEquals(ServerVersion.V_1_9)
            && version.isOlderThan(ServerVersion.V_1_14)) {
      ...
      return new NpcTypeImpl(
          name, type, hologramOffset, new HashSet<>(allowedProperties), defaultProperties);
    }
  }
}
